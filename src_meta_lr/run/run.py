...
def evaluate_sequential(args, runner, logger, load_time_step=0):
    ...
    for i in range(n_scen):
        run_args = {'test_mode': True, 'vid_writer': vw,
                    'test_scen': True, 'domain_n': args.eval_domain}
        if args.eval_all_scen:
            run_args['index'] = i
...
def run_sequential(args, logger):
    ...
    meta_lr_scheme = getattr(args, 'meta_lr_scheme', None)
    ...
    source_buffer =  ReplayBuffer(scheme, groups, args.buffer_size, env_info["episode_limit"] + 1,
                        preprocess=preprocess,
                        device="cpu" if args.buffer_cpu_only else args.device)

    target_buffer =  ReplayBuffer(scheme, groups, args.buffer_size, env_info["episode_limit"] + 1,
                        preprocess=preprocess,
                        device="cpu" if args.buffer_cpu_only else args.device)
    ...
    while runner.t_env <= args.t_max:
        ...
        if args.runner == 'total_episode' and buffer.can_sample(runner.batch_size):
            ...
            if meta_lr_scheme == "MLDG":
                episode_batch = runner.run(test_mode=False, meta_lr_scheme=meta_lr_scheme)
                buffer.insert_episode_batch(episode_batch)
                insert_buffer_num += runner.batch_size
            
            elif meta_lr_scheme == "DG-MAML":
                episode_batch_source = runner.run(test_mode=False, meta_lr_scheme=meta_lr_scheme, meta_test_mode=False)
                episode_batch_target = runner.run(test_mode=False, meta_lr_scheme=meta_lr_scheme, meta_test_mode=True)
                source_buffer.insert_episode_batch(episode_batch_source)
                target_buffer.insert_episode_batch(episode_batch_target)
                insert_buffer_num += runner.batch_size
        ...
        if buffer.can_sample(args.batch_size) or (source_buffer.can_sample(args.batch_size) and target_buffer.can_sample(args.batch_size)):
            for _ in range(args.training_iters):
                ...
                if meta_lr_scheme == "MLDG":
                    ...
                    learner.train_MLDG(episode_sample, runner, runner.t_env, episode, 
                                       scheme=scheme, groups=groups, buffer_size=args.buffer_size, max_seq_length=env_info["episode_limit"] + 1, 
                                       preprocess=preprocess, device="cpu" if args.buffer_cpu_only else args.device)

                elif meta_lr_scheme == "DG-MAML":
                    source_sample = source_buffer.sample(args.batch_size)
                    src_max_ep_t = source_sample.max_t_filled()
                    source_sample = source_sample[:, :src_max_ep_t]

                    target_sample = target_buffer.sample(args.batch_size)
                    dst_max_ep_t = target_sample.max_t_filled()
                    target_sample = target_sample[:, :dst_max_ep_t]

                    if source_sample.device != args.device:
                        source_sample.to(args.device)
                    
                    if target_sample.device != args.device:
                        target_sample.to(args.device)
                    
                    learner.train_DGMAML(source_sample, target_sample, runner.t_env, episode)
            ...
            for empirical_study_n in range(3): 
                for _ in range(n_test_runs):
                    if args.runner == 'total_episode':
                        ...
                        runner.run(test_mode=True, domain_n = empirical_study_n, meta_lr_scheme=meta_lr_scheme)
