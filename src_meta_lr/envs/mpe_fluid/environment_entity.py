...
class MultiAgentFluidEnv(gym.Env, MultiAgentEnv):
    ...
    def reset(self, constrain_num=None, test=False, index=None, domain_n = 0, meta_lr_scheme=None, meta_test_mode=False):
        # reset world
        self.reset_callback(self.world, np_random = np.random , test=test, domain_n=domain_n, meta_lr_scheme=meta_lr_scheme, meta_test_mode=meta_test_mode)
        # reset renderer
        self._reset_render()
    ...
    def render(self, mode='human', srk=[1,2,4], sr_color=["#2CAFAC","#FB5607", "#1982C4"]):
        ...
        for entity in self.world.entities:
            ...
            if 'agent' in entity.name:
                geom.set_color(*entity.color, alpha=0.9)
            else:
