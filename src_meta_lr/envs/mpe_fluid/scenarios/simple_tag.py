    ...
    def reset_world(self, world, np_random, test, domain_n, meta_lr_scheme, meta_test_mode):
        ...
        if (test == False) or (test == True and domain_n == 0):
             if meta_lr_scheme == "MLDG" or meta_lr_scheme == 'DG-MAML':
                ...
                self.MLDG_initial_advs(world, np_random, self.scenario_config["num_adversaries"], meta_test_mode)
                
#             self.random_initial_agents(world, np_random, self.scenario_config["num_agents"])
#             self.random_initial_advs(world, np_random, self.scenario_config["num_adversaries"])
    ...
    def MLDG_initial_advs(self, world, np_random, config, meta_test_mode):
        if len(config) == 2:
            if meta_test_mode:
                initial_num = config[-1]
            else:
