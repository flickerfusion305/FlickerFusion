    ...
    def reset_world(self, world, np_random, test, domain_n, meta_lr_scheme, meta_test_mode):
        ...
        if not test: 
            if config['dropout'] and config['dropout']['train']: # uniform sample in combinatorics
                ...
                agentnum = self.MLDG_initial_agents(world, np_random, self.scenario_config["num_agents"], meta_test_mode)
                advnum = self.MLDG_initial_advs(world, np_random, self.scenario_config["num_adversaries"], meta_test_mode)
