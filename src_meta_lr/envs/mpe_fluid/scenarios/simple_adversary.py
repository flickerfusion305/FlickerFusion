...
#                     world.maxobscnt_dict["adv"] = num_adversaries[1] + scenario_config['intra_trajectory'][1]  # max In Domain entity num (max init + max intra)
    ...
    def reset_world(self, world, np_random, test, domain_n, meta_lr_scheme, meta_test_mode):
        ...
        if not test: 
            agentnum = self.MLDG_initial_agents(world, np_random, self.scenario_config["num_agents"], meta_test_mode) + self.scenario_config["num_agents"][0]
            advnum = self.MLDG_initial_advs(world, np_random, self.scenario_config["num_agents"], meta_test_mode) + self.scenario_config["num_adversaries"][0]
            targetnum = self.MLDG_initial_targets(world, np_random, config["num_targets"],meta_test_mode) + self.scenario_config["num_targets"][0]
            decoynum = self.MLDG_initial_decoys(world, np_random, config["num_decoys"],meta_test_mode) + self.scenario_config["num_decoys"][0]
            world.DOnum = {}
    ...
    def MLDG_initial_decoys(self, world, np_random, config, meta_test_mode):
        if len(config) == 2:
            if meta_test_mode:
                initial_num = config[-1]
            else:
