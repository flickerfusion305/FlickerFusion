...
class QLearner:
    ...
    def train_MLDG(self, meta_train_batch: EpisodeBatch, runner: ParallelRunner, t_env: int, episode_num: int, **kwargs):
        meta_train_loss = self._compute_loss(meta_train_batch)
        original_params = [p.clone() for p in self.params]

        self._temporary_update(meta_train_loss, alpha=self.args.meta_alpha)
        
        meta_test_buffer = ReplayBuffer(kwargs['scheme'], kwargs['groups'], kwargs['buffer_size'], 
                                        kwargs['max_seq_length'], kwargs['preprocess'], kwargs['device'])
        
        while not meta_test_buffer.can_sample(self.args.batch_size):
            meta_test_episode_batch = runner.run(test_mode=False, meta_lr_scheme=self.args.meta_lr_scheme, meta_test_mode=True)
            meta_test_buffer.insert_episode_batch(meta_test_episode_batch)
        
        meta_test_batch = meta_test_buffer.sample(self.args.batch_size)
        
        if meta_test_batch.device != self.args.device:
            meta_test_batch.to(self.args.device)

        meta_test_loss = self._compute_loss(meta_test_batch)

        final_loss = self.args.meta_gamma * (meta_train_loss + self.args.meta_beta * meta_test_loss)

        for original_p, p in zip(original_params, self.params):
        ...
        # Logging
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            self.logger.log_stat("meta_train_loss", meta_train_loss.item(), t_env)
            self.logger.log_stat("meta_test_loss", meta_test_loss.item(), t_env)
            self.logger.log_stat("final_loss", final_loss.item(), t_env)
            self.logger.log_stat("grad_norm", grad_norm, t_env)
            self.log_stats_t = t_env

    def train_DGMAML(self, train_batch_1: EpisodeBatch, train_batch_2: EpisodeBatch, t_env: int, episode_num: int):
        # Step 1: Compute loss on train_batch_1
        meta_train_loss = self._compute_loss(train_batch_1)
        ...
        meta_train_grad = [p.grad.clone() if p.grad is not None else None for p in self.params]

        with th.no_grad():
            for i, (p, g) in enumerate(zip(self.params, meta_train_grad)):
                if g is not None:
                    p.data -= self.args.meta_alpha * g

        meta_test_loss = self._compute_loss(train_batch_2)
        ...
        # Step 4: Final backward pass and parameter update
        self.optimiser.zero_grad()
        final_loss.backward()
        grad_norm = th.nn.utils.clip_grad_norm_(self.params, self.args.grad_norm_clip)
        self.optimiser.step()
    ...
    def _temporary_update(self, loss, alpha):
        # Temporarily update parameters using the meta-train loss
        self.optimiser.zero_grad()
