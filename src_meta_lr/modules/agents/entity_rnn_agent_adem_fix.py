...
#args.repeat_mlp: depth
#args.lamda_embed_dim : lamda_vector 지정여부
#args.lamda_embed_dim_num :default num =te 
#args.lamda_encoder : encoder ,decoder 사용여부
#args.domain_proxy: domain proxy 사용여부
...
class EntityAttentionRNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        #! if speread then te=2
        te=(len(args.env_args['scenario_config'])-2)//2 #entity type의 수
        
        #!
        if args.lamda_encoder: 
            if args.lamda_embed_dim   : #en
                self.fc0 = nn.Linear(te, args.lamda_embed_dim_num) #(2) 수동으로 정해줌
                self.fc1 = nn.Linear(args.lamda_embed_dim_num+(input_shape-te), args.attn_embed_dim)
                
                
            else :
                self.fc0 = nn.Linear(te, te) #(1)same size as lambda vector
                self.fc1 = nn.Linear(input_shape, args.attn_embed_dim)
        else :        
            self.fc1 = nn.Linear(input_shape, args.attn_embed_dim)

        # EntityAttentionLayer (self, in_dim, embed_dim, out_dim, args)
        ...
        if args.domain_proxy:
            self.fc2 = nn.Linear((args.attn_embed_dim+te)*(1+args.self_loc+args.double_attn), args.rnn_hidden_dim)
        else: 
    ...
    def init_hidden(self):
        ...
        self.attn_weights=None
        return self.fc0.weight.new(1, self.args.rnn_hidden_dim).zero_()
    ...
    def forward(self, inputs, hidden_state, ret_attn_logits=None, msg=None, ret_attn_weights=False):
        ...
        #!
        te=(len(self.args.env_args['scenario_config'])-2)//2
        #여기다 기본 domain proxy를 넣을까 고민
        ...
        #! mlp 받은대로 처리. repeat_mlp=depth, lamda_embed_dim=width
        if self.args.lamda_encoder:
            for _ in range(self.args.repeat_mlp):
                x0 = F.relu(self.fc0(lamda))
                lamda = x0
            
            #!concat
            lamda_obs=th.cat([x0,obs],dim=2) 
            
            for _ in range(self.args.repeat_mlp):
                x1 = F.relu(self.fc1(lamda_obs))
                lamda_obs = x1
        else :
        ...
        #! 
        if self.args.domain_proxy:    
            aa,bb,_=x2.shape
            proxy_data=sum(entities[0,:,:te])
            domain_proxy = proxy_data.unsqueeze(0).unsqueeze(0).expand(aa, bb, -1)
            x2= th.cat([x2, domain_proxy], dim=2)
