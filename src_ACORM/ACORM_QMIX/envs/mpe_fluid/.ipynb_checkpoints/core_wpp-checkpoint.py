...
class World(object):
    ...
    def get_collision_force(self, entity_a, entity_b):
        ...
        if entity_a in self.agents and entity_b not in self.agents:
            force = entity_a.pushing_force * delta_pos / dist * penetration if dist <= dist_min else self.contact_force * delta_pos / dist * penetration
            if entity_a.movable:
                force_a = +force / (entity_a.pushing_force/2) if not entity_a.small else +force
            else:
                force_a = None
            force_b = -force if entity_b.movable else None
        elif entity_a not in self.agents and entity_b in self.agents:
            force = entity_b.pushing_force * delta_pos / dist * penetration if dist <= dist_min else self.contact_force * delta_pos / dist * penetration
            if entity_a.movable:
                force_a = +force / (entity_a.pushing_force/2) if not entity_a.small else +force
            else:
                force_a = None
    ...
    def boundary_collision(self, entity):
        ...
        for truck in self.trucks:
            boundary_points = truck
            # boundary_points = [point for point in self.entities if "truck" in point.name]
            pos_point = boundary_points[0].state.p_pos
            neg_point = [-p for p in pos_point]
            # neg_point = boundary_points[0].state.p_pos
