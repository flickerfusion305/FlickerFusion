...
class MultiAgentFluidEnv(gym.Env, MultiAgentEnv):
    ...
    def step(self, action_n,scenario_config={}): #add scenario_config
    ...
    def reset(self, constrain_num=None, test=False, index=None):
        # reset world
        self.reset_callback(self.world, np_random = np.random)
        # reset renderer
        self._reset_render()
    ...
    def get_env_info(self, args):
        ...
        if getattr(args, 'obs_type', None) == "QMIX_M":
            env_info["state_shape"] = self.world.state_shape + self.world.max_n_entities
            env_info["obs_shape"] = self.world.obs_shape + self.world.max_n_entities
    ...
    def render(self, mode='human', srk=[1,2,4], sr_color=["#2CAFAC","#FB5607", "#1982C4"]):
        ...
        for entity in self.world.landmarks:
            if "truck" in entity.name:
                boundary, boundary_xform = self.draw_boundary()
                boundary.set_color(*np.array([0, 0, 0]), alpha=0.5)
                self.children_geoms.append(boundary)
                self.children_geoms_xform.append(boundary_xform)
                break
    ...
    def draw_boundary(self):
        ...
        for truck in self.world.trucks:
            ...
            pos_x, pos_y = boundary_points[0].state.p_pos
            neg_x = -pos_x
            neg_y = -pos_y

            # Dashed line
            dashed_line, _ = self.create_dashed_line((pos_x, pos_y), (pos_x, neg_y), dash_length=0.03)
            lines.extend(dashed_line.gs)
            ...
            lines.extend([solid_line1, solid_line2, solid_line3])
