...
class Scenario(BaseScenario):
    def make_world(self, scenario_config={}):
        ...
        if 'num_landmarks' in scenario_config.keys():
            ...
            num_landmarks = np.random.randint(start, end)
            if self.scenario_config["dropout"]:
                if not self.scenario_config["dropout"]["inference"] or not self.scenario_config["domain_aware"]:
                    world.maxobscnt_dict["target"] = scenario_config['max_n_landmarks']
                else:
        ...
        if self.scenario_config["dropout"]:
            if not self.scenario_config["dropout"]["inference"]:
                world.obs_shape = (world.max_n_agents + world.max_n_landmarks) * (2+4)
            else:
    ...
    def reset_world(self, world, np_random, test, domain_n):
        ...
        if not test: 
            if config['dropout'] and config['dropout']['train']: # uniform sample in combinatorics
                ...
                targetintraDOsamples = np.arange(config['dropout']['target'][num_targets][1]+1)
                ...
                targetintraDOnum = np_random.choice(targetintraDOsamples)

                agentnum = self.random_initial_agents(world, np_random, [num_agents])
                targetnum = self.random_initial_landmarks(world, np_random, [num_targets])
                world.DOnum = {"agent": {agentnum: agentinitDOnum, agentnum+1: agentinitDOnum + agentintraDOnum},
                "target": {targetnum: targetinitDOnum, targetnum+1: targetintraDOnum + targetinitDOnum}}

            else:
                agentnum = self.random_initial_agents(world, np_random, config["num_agents"])
                targetnum = self.random_initial_landmarks(world, np_random, config["num_landmarks"])
                world.DOnum = {}
            

        if test: # uniform sample in range
            IDmaxagent = config["num_agents"][-1] + config["intra_trajectory"][0]
            IDmaxtarget = config["num_landmarks"][-1] + config["intra_trajectory"][1]
            if domain_n == 0:
                config = self.scenario_config
            elif domain_n == 1:
            ...
            ##for Attention matrix
                ...
                config["num_landmarks"][0] += config["intra_trajectory"][1]
