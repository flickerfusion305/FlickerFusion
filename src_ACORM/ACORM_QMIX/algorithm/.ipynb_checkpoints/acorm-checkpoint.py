...
#         self.mix_input_dim = args.state_embed_dim + args.N * args.att_out_dim
        self.mix_input_dim = args.N *(args.state_embed_dim + args.att_out_dim)
        ...
        self.state_fc = nn.Linear(args.state_dim, args.state_embed_dim)
        ...
        if self.hyper_layers_num == 2:
            ...
            print((self.args.state_dim, self.args.N, self.args.att_out_dim))
    ...
    def forward(self, q, s, att):
        ...
        s = s.reshape(-1, self.args.N*self.args.state_embed_dim)  # (batch_size * max_episode_len, state_dim)
...
class ACORM_Agent(object):
    ...
    def update_qmix(self, inputs, batch_o, batch_s, batch_r, batch_a, batch_last_a, batch_avail_a_n, batch_active, batch_dw, max_episode_len):
        ...
        fc_batch_s = F.relu(self.eval_mix_net.state_fc(batch_s.reshape(-1, self.state_dim))).reshape(-1, max_episode_len+1, self.args.state_embed_dim)    # shape(batch*max_len+1, state_dim)
        
        # evaluate Q network and calculate target value
        ...
        # argmax for selecting action value
        with torch.no_grad():
        ...
        # role_embeddings
        role_embeddings = role_embeddings[:, :-1].reshape(-1, self.N, self.role_embedding_dim)  # shape=(batch_size * max_episode_len, N, role_embed_dim)

        # calculate att_eval with attention network
        att_eval = self.eval_mix_net.attention_net(fc_batch_s[:, :-1], role_embeddings, role_embeddings).reshape(-1, max_episode_len, self.N * self.att_out_dim)  # ((batch*max_episode_len), N, att_dim)->(batch, len, N*att_dim)
        with torch.no_grad():
            att_target = self.target_mix_net.attention_net(fc_batch_s[:, 1:], role_embeddings, role_embeddings).reshape(-1, max_episode_len, self.N * self.att_out_dim)  # ((batch*max_episode_len), N, att_dim)->(batch, len, N*att_dim)

        # calculate Q total
        q_total_eval = self.eval_mix_net(q_evals, fc_batch_s[:, :-1], att_eval)
        q_total_target = self.target_mix_net(q_targets, fc_batch_s[:, 1:], att_target)
