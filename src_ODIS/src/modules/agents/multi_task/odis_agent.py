...
#TODO mpe_decomposer를 통해 구한 dimension 들로 Encoder, Decoder들 차원 맞추기, 각 내용은 StateEncoder, ObsEncoder, Encoder, Decoder에 있습니다
...
class StateEncoder(nn.Module):
    def __init__(self, task2input_shape_info, task2decomposer, task2n_agents, decomposer, args):
        ...
        self.n_actions = task2decomposer_.n_actions #NOTE MPE env

        

        # define state information processor
        if self.state_last_action: # Not used
            self.ally_encoder = nn.Linear(state_nf_al + (self.n_actions), self.entity_embed_dim) 
            self.enemy_encoder = nn.Linear(state_nf_en, self.entity_embed_dim) 
            # state_nf_al += self.n_actions_no_attack + 1
        else:
            self.ally_encoder = nn.Linear(state_nf_al + (self.n_actions), self.entity_embed_dim) # NOTE MPE env
            self.enemy_encoder = nn.Linear(state_nf_en, self.entity_embed_dim) # NOTE MPE env
    ...
    def forward(self, states, hidden_state, task, actions=None):
        ...
        # NOTE MPE env
        current_compact_action_states = task_decomposer.decompose_action_info(F.one_hot(actions.reshape(-1), num_classes=self.task2last_action_shape[task]))
        current_compact_action_states = current_compact_action_states.reshape(bs, n_agents, -1).permute(1, 0, 2).unsqueeze(2)
        ally_states = th.cat([ally_states, current_compact_action_states], dim=-1) # NOTE MPE env
        # current_attack_action_info = current_attack_action_info.reshape(bs, n_agents, n_enemies).sum(dim=1) # NOTE MPE env
        #attack_action_states = (current_attack_action_info > 0).type(ally_states.dtype).reshape(bs, n_enemies, 1, 1).permute(1, 0, 2, 3) # NOTE MPE env
        enemy_states = th.stack(enemy_states, dim=0)  # [n_enemies, bs, 1, state_nf_en]
        #enemy_states = th.cat([enemy_states, attack_action_states], dim=-1) # NOTE MPE env

        # stack action information
        if self.state_last_action:
            last_action_states = th.stack(last_action_states, dim=0)
            compact_action_states= task_decomposer.decompose_action_info(last_action_states)
...
class ObsEncoder(nn.Module):
    ...
    def __init__(self, task2input_shape_info, task2decomposer, task2n_agents, decomposer, args):
        ...
        self.entity_embed_dim = args.entity_embed_dim
        self.attn_embed_dim = args.attn_embed_dim
        obs_en_dim = decomposer.obs_en_dim
        obs_al_dim = decomposer.obs_al_dim
        ## get wrapped obs_own_dim
        wrapped_obs_own_dim = decomposer.wrapped_obs_own_dim
    ...
    def forward(self, inputs, hidden_state, task):
        ...
        # embed agent_id inputs and decompose last_action_inputs NOTE No agent id for MPE
        # agent_id_inputs = [
        ...
        action_info = task_decomposer.decompose_action_info(last_action_inputs)

        # incorporate agent_id embed and compact_action_states
        # own_obs = th.cat([own_obs, agent_id_inputs, compact_action_states], dim=-1)
        own_obs = th.cat([own_obs, agent_id_inputs, action_info], dim=-1)
        ...
        enemy_feats = th.stack(enemy_feats, dim=0) # NOTE MPE env
...
class Decoder(nn.Module):
    ...
    def __init__(self, task2input_shape_info, task2decomposer, task2n_agents, decomposer, args):
        ...
        self.q_skill = nn.Linear(self.entity_embed_dim + self.entity_embed_dim, n_actions)
    ...
    def forward(self, inputs, hidden_state, task, skill):
        ...
        q = th.cat([q_base], dim=-1) # NOTE MPE env
