...
class EpisodeRunner:
    ...
    def reset(self, test_mode = False,  domain_n=0):
        self.batch = self.new_batch()
        self.env.reset(test=test_mode, domain_n=domain_n)
        self.t = 0

    def run(self, test_mode=False, nolog=False, pretrain=False, domain_n = 0):
        self.reset(test_mode = test_mode, domain_n = domain_n)
        ...
        while not terminated:
            ...
            reward, terminated, _, env_info = self.env.step(actions[0])
        ...
        if test_mode and len(self.test_returns) == self.args.test_nepisode and self.args.save_gif:
