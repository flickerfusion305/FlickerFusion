...
def evaluate_sequential(main_args, logger, task2runner):
    ...
    with th.no_grad():
        for task in main_args.test_tasks: # TEST 3 Domains
            if main_args.evaluate: 
                for _ in range(n_test_runs):
                    task2runner[task].run(test_mode=True, domain_n = main_args.eval_domain)
                os._exit(0)
            else: 
                for empirical_study_n in range(3): 
                    logger.console_logger.info("Testing domain_{}".format(empirical_study_n))
                    for _ in range(n_test_runs):
                        task2runner[task].run(test_mode=True, domain_n = empirical_study_n)
...
def init_tasks(task_list, main_args, logger):
    ...
    return task2args, task2runner, task2buffer, task2scheme, task2groups, task2preprocess, env_info
...
def train_sequential(train_tasks, main_args, logger, learner, task2args, task2runner, task2offlinedata, t_start=0,
    ...
    with th.no_grad():
        for task in main_args.test_tasks:
            ...
            for empirical_study_n in range(3): # 3 domain test
                logger.console_logger.info("Testing domain_{}".format(empirical_study_n))
                for _ in range(n_test_runs):
                    task2runner[task].run(test_mode=True, pretrain=pretrain, domain_n = empirical_study_n)
            # for _ in range(n_test_runs):
...
def run_sequential(args, logger):
    ...
    task2args, task2runner, task2buffer, task2scheme, task2groups, task2preprocess, env_info = init_tasks(all_tasks, main_args,
                                                                                                logger)
    ...
    mac = mac_REGISTRY[main_args.mac](train_tasks=all_tasks, task2scheme=task2buffer_scheme, task2args=task2args,
                                      main_args=main_args, env_info=env_info)
    ...
    for task in main_args.train_tasks:
        # create offline data buffer
        task2offlinedata[task] = OfflineBuffer(task, main_args.train_tasks_data_quality[task],
                                               data_folder=main_args.offline_data_name,
                                               offline_data_size=args.offline_data_size,
                                               random_sample=args.offline_data_shuffle)
