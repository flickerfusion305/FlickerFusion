...
class UPDeT(nn.Module):
    def __init__(self, input_shape, args):
        ...
        self.args = args
        self.transformer = Transformer(args.obs_dim, args.emb, args.heads, args.depth, args.emb)
        self.q_basic = nn.Linear(args.emb, 5)
    ...
    def forward(self, inputs, hidden_state, mask):
        outputs, _ = self.transformer.forward(inputs, hidden_state, mask)
        ...
        # each enemy has an output Q NOTE No enemy action for mpe_fluid env
        ...
        # concat enemy Q over all enemies NOTE No enemy action for mpe_fluid env
        # q_enemies = torch.stack(q_enemies_list, dim=1).squeeze()

        # concat basic action Q with enemy attack Q NOTE No enemy action for mpe_fluid env
        # q = torch.cat((q_basic_actions), 1)
        q = q_basic_actions
        return q, h
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # mask size: bs, na, t NOTE added for mpe_fluid env
        bs, na, tsz = mask.shape
        mask = mask.repeat(h, 1, tsz).reshape(bs * na * h, tsz, tsz)

        if mask is not None:
            dot = dot.masked_fill(mask == 1, -1e9) # if mask = 1, mask out
...
if __name__ == '__main__':
    ...
    # testing the agent
    agent = UPDeT(None, args)
    hidden_state = agent.init_hidden().expand(args.ally_num, 1, -1)
    tensor = torch.rand(args.ally_num, args.ally_num+args.enemy_num, args.token_dim)
    q_list = []
