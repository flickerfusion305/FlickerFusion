...
class BasicMAC:
    ...
    def forward(self, ep_batch, t, test_mode=False):
        ...
        # transformer based agent
            ...
            agent_inputs, obs_entity_mask = self._build_inputs_transformer(ep_batch, t)
    ...
    def _build_inputs_transformer(self, batch, t):
        ...
        inputs = []
        raw_obs = batch["obs"][:, t]
        obs_entity_mask = batch["obs_entity_mask"][:,t]
        arranged_obs = th.cat((raw_obs[:, :, -1:], raw_obs[:, :, :-1]), 2)
        reshaped_obs = arranged_obs.view(-1, self.args.n_entities, self.args.obs_dim)
        inputs.append(reshaped_obs)
        #inputs = th.cat(inputs, dim=1)
        inputs = th.cat(inputs, dim=1).cuda()
        return inputs, obs_entity_mask
