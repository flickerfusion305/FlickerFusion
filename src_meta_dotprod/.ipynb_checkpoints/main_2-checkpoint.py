...
def run_experiment(params):
    ...
    if not any(["--env-config" in s for s in params]):
        params.append("--env-config=mpe_fluid")
...
if __name__ == '__main__':
    ...
    configurations = [
        ["--env-config=spread_mpev2", "--config=meta_learnings/qatten_123456_DOTPROD"],
#         ["--env-config=mpe_fluid", "--config=qmix_m"],
        # You can append more combinations of configurations
    ]

    processes = []
    for config in configurations:
        p = Process(target=run_experiment, args=(config,))
        p.start()
        processes.append(p)
