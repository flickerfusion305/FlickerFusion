...
def run(_run, _config, _log):
    ...
    # unique_token += "{}_{}_{}".format(args.obs_type, args.env_args['scenario_id'][:-3], datetime.datetime.now().strftime("%m%d_%H%M_%S%f"))
    unique_token += "{}_{}_{}".format(args.name, args.env_args['scenario_id'][:-3], args.meta_lr_scheme)
...
def run_sequential(args, logger):
    ...
    if args.meta_lr_scheme == "SMLDG" or args.meta_lr_scheme == "DOTPROD":
        number_of_domains = 9
        if args.env_args["scenario_id"] == "simple_guard.py":
            number_of_domains = 6
        elif args.env_args["scenario_id"] == "simple_adversary.py":
            number_of_domains = 36

        buffer_list = []
        for i in range(number_of_domains):       
            buffer_list.append(ReplayBuffer(scheme, groups, args.buffer_size, env_info["episode_limit"] + 1,
    ...
    # Learner
    learner = le_REGISTRY[args.learner](mac, buffer_list[0].scheme, logger, args)
    ...
    while runner.t_env <= args.t_max:
        ...
        if args.runner == 'total_episode' and buffer.can_sample(runner.batch_size):
            ...
            if meta_lr_scheme == "SMLDG" or meta_lr_scheme == "DOTPROD":
                for i in range(number_of_domains):
                    episode_batch = runner.run(test_mode=False, meta_lr_scheme=meta_lr_scheme, domain_type=i)
                    buffer_list[i].insert_episode_batch(episode_batch)
        ...
        for i in range(len(buffer_list)):
            can_sample_list.append(buffer_list[i].can_sample(args.batch_size))
        
        if all(can_sample_list):
            for _ in range(args.training_iters):
                ...
                if meta_lr_scheme == "SMLDG":
                    learner.train_SMLDG(source_domains=buffer_list, t_env=runner.t_env, episode_num=episode)
                
                elif meta_lr_scheme == "DOTPROD":
                    learner.train_meta_dotprod(buffer_list, runner.t_env, episode)
            ...
            for empirical_study_n in range(3): 
                ...
                for _ in range(n_test_runs):
                    if args.runner == 'total_episode':
                        ...
                        test_type = np.random.randint(0, number_of_domains)
                        runner.run(test_mode=True, domain_n = empirical_study_n, meta_lr_scheme=meta_lr_scheme, domain_type=test_type)
