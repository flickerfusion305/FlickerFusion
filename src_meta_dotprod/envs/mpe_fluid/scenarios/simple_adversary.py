...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, np_random, test, domain_n, meta_lr_scheme, domain_type):
        ...
        if (test == False) or (test == True and domain_n == 0):
            self.random_initial_world(world, np_random, self.scenario_config["num_agents"], self.scenario_config["num_adversaries"], self.scenario_config["num_targets"], self.scenario_config["num_decoys"], domain_type)
        # set OOD config for evalutation
            ...
            self.random_initial_adversaries(world, np_random, test_config["num_adversaries"])
            self.random_initial_targets(world, np_random, test_config["num_targets"])
            self.random_initial_decoys(world, np_random, test_config["num_decoys"])
    ...
    def possible_max_dist(self, world, target):
        # x, y boundary: [-1, 1]
        if target.state.p_pos[0] >= 0 and target.state.p_pos[1] >= 0:
            delta_pos = target.state.p_pos - np.array([-world.boundary, -world.boundary])
    ...
    def random_initial_world(self, world, np_random, agent_config, adv_config, target_config, decoy_config, domain_type):
        agent_start, agent_end = agent_config
        adv_start, adv_end = adv_config
        target_start, target_end = target_config
        decoy_start, decoy_end = decoy_config
        possible_agent_list = list(range(agent_start, agent_end + 1)) 
        possible_adv_list = list(range(adv_start, adv_end+1))
        possible_target_list = list(range(target_start, target_end+1))
        possible_decoy_list = list(range(decoy_start, decoy_end+1))
        
        num_agent = len(possible_agent_list)
        num_adv = len(possible_adv_list)
        num_target = len(possible_target_list) 
        num_decoy = len(possible_decoy_list) 
        agent_idx = (domain_type // (num_adv * num_target * num_decoy)) % num_agent
        adv_idx = (domain_type // (num_target * num_decoy)) % num_adv
        target_idx = (domain_type // num_decoy) % num_target
        decoy_idx = domain_type % num_decoy
        
        for i in range(possible_agent_list[agent_idx]): 
            agent = self.create_new_agent(world)
            world.agents.append(agent)
        for i in range(possible_adv_list[adv_idx]): 
            agent = self.create_new_adv(world) 
            world.agents.append(agent)
        for i in range(possible_target_list[target_idx]):
            target = self.create_new_target(world) 
            world.landmarks.append(target)
        for i in range(possible_decoy_list[decoy_idx]): 
            decoy = self.create_new_decoy(world)
            world.landmarks.append(decoy)
