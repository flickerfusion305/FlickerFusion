...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, np_random, test, domain_n, meta_lr_scheme, domain_type):
        ...
        if not test: 
            if config['dropout'] and config['dropout']['train']: # uniform sample in combinatorics
                ...
                agentnum, advnum = self.random_initial_world(world, np_random, self.scenario_config["num_agents"], self.scenario_config["num_adversaries"], domain_type)
                world.DOnum = {}
            

        if test: # uniform sample in range
            IDmaxagent = config["num_agents"][-1] + 1
            IDmaxadv = config["num_adversaries"][-1] + 1
            if domain_n == 0:
                config = self.scenario_config
            elif domain_n == 1:
    ...
    def random_initial_world(self, world, np_random, agent_config, landmark_config, domain_type):
        ...
        return agent_initial_num, landmark_initial_num
