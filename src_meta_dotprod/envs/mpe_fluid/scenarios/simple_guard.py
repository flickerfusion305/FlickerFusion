...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, np_random, test, domain_n, meta_lr_scheme, domain_type):
        ...
        if (test == False) or (test == True and domain_n == 0):
            self.random_initial_world(world, np_random, self.scenario_config["num_agents"], self.scenario_config["num_targets"], domain_type)
    ...
    def random_initial_world(self, world, np_random, agent_config, target_config, domain_type):
        domain_type += 1
        ...
        if 1 <= domain_type <= 2:
            ...
            if domain_type == 1:
                target_initial_num = possible_target_list[0]
            elif domain_type == 2:
                target_initial_num = possible_target_list[1]
        ...
        for i in range(target_initial_num):
            agent = Agent()
            agent.target = True
            agent.name = f"target_{world.target_count}" 
            agent.collide = True
        ...
        return agent_initial_num, target_initial_num
