...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, np_random, test, domain_n, meta_lr_scheme, domain_type):
        ...
        if (test == False) or (test == True and domain_n == 0):
            ...
            self.random_initial_world(world, np_random, self.scenario_config["num_agents"], self.scenario_config["num_landmarks"], domain_type)
    ...
    def random_initial_world(self, world, np_random, agent_config, landmark_config, domain_type):
        domain_type += 1
        agent_start, agent_end = agent_config
        landmark_start, landmark_end = landmark_config

        possible_agent_list = list(range(agent_start, agent_end + 1))
        possible_landmark_list = list(range(landmark_start, landmark_end + 1))

        if 1 <= domain_type <= 3:
            agent_initial_num = possible_agent_list[0]

            if domain_type == 1:
                landmark_initial_num = possible_landmark_list[0]
            elif domain_type == 2:
                landmark_initial_num = possible_landmark_list[1]
            else:
                landmark_initial_num = possible_landmark_list[2]
        ...
        for i in range(landmark_initial_num):
            landmark = Landmark()
            landmark.name = f"target_{world.landmark_count}"
            landmark.collide = False
            landmark.movable = False
