...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, np_random, test, domain_n, meta_lr_scheme, domain_type):
        ...
        if (test == False) or (test == True and domain_n == 0):
            # self.random_initial_agents(world, np_random, self.scenario_config["num_agents"])
            # self.random_initial_landmarks(world, np_random, self.scenario_config["num_landmarks"])
            self.random_initial_world(world, np_random, self.scenario_config["num_agents"], self.scenario_config["num_adversaries"], domain_type)
        # set OOD config for evalutation
            ...
            self.random_initial_advs(world, np_random, test_config["num_adversaries"])
        ...
        for i, agent in enumerate(world.agents):
            ...
            for _ in range(max_attempts):
                if (agent.adversary):
                    x_pos = np_random.uniform(-world.boundary, -world.boundary * 0.6)
                    y_pos = np_random.uniform(world.boundary * 0.6, world.boundary)
                else:
    ...
    def possible_max_dist(self, from_entity, to_entity):
    ...
    #             if not a is agent and self.is_collision(a, agent):
    #                 rew -= 10
    ...
    #                 rew -= 50
    #             dist = np.sqrt(np.sum(np.square(a.state.p_pos - adv.state.p_pos))) - adv.size - a.size
    #             max_dist = self.possible_max_dist(adv, a)
    #             norm_dist = (dist / max_dist)*20
    #             norm_dists.append(norm_dist)
    #         min_dists.append(min(norm_dists))
    ...
    def add_new_agent(self, world, np_random): 
        ...
        self.set_pos_random_good(agent, world.boundary*0.98, np_random)
        agent.state.p_vel = np.zeros(world.dim_p)
    ...
    def add_new_adv(self, world, np_random):
        ...
        self.set_pos_random_adv(agent, world.boundary*0.98, np_random)
        agent.state.p_vel = np.zeros(world.dim_p)
    ...
    def set_pos_random_adv(self, entity, boundary, np_random): 
    ...
    def set_pos_random_good(self, entity, boundary, np_random): 
        edge = np_random.randint(2)
        if edge == 0:  # d
            x = np_random.uniform(-boundary, boundary)
            y = -boundary
    ...
    def random_initial_world(self, world, np_random, agent_config, adv_config, domain_type):
        domain_type += 1
        agent_start, agent_end = agent_config
        adv_start, adv_end = adv_config

        possible_agent_list = list(range(agent_start, agent_end + 1))
        possible_adversary_list = list(range(adv_start, adv_end + 1))

        if 1 <= domain_type <= 3:
            agent_initial_num = possible_agent_list[0]

            if domain_type == 1:
                adversary_initial_num = possible_adversary_list[0]
            elif domain_type == 2:
                adversary_initial_num = possible_adversary_list[1]
            else:
                adversary_initial_num = possible_adversary_list[2]


        if 4 <= domain_type <= 6:
            agent_initial_num = possible_agent_list[1]
        ...
        if 7 <= domain_type <= 9:
            agent_initial_num = possible_agent_list[2]
        ...
        for i in range(agent_initial_num):
            agent = Agent()
            agent.name = f"agent_{world.agent_count}"
            agent.adversary = False
            agent.collide = False
        ...
        for i in range(adversary_initial_num):
            agent = Agent()
            agent.name = f"adv_{world.agent_count}"
            agent.adversary = True
            agent.collide = False
            agent.silent = True
            agent.size = 0.04
            ...
            world.adv_count += 1
            world.all_agents[agent.name] = 0
            world.adversaries.append(agent)