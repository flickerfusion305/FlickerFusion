...
from components.transforms import process_agent_obs, process_state, process_entities, process_DAMOED
...
class ParallelRunner:
    ...
    def run(self, test_mode=False, test_scen=None, index=None, vid_writer=None, constrain_num=None, domain_n =0, 
            meta_lr_scheme=None, domain_type=None, runtime_duplicated=False):
        """
        ...
        if self.args.env == "traffic_junction":
            ...
            self.reset(test=test_scen, index=index, constrain_num=constrain_num, domain_n = domain_n, meta_lr_scheme=meta_lr_scheme, domain_type=domain_type)
        ...
        while True:
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    if not test_mode and not runtime_duplicated:
    ...
    while True:
        ...
        if cmd == "step":
            ...
            if entity_scheme: # REFIL / CAMA
                ...
                if args.entity_dropout and (args.dropout_type == "damoed" or args.dropout_type == "random"):
                    dropout_obs_mask, entity_dropout, entity_combination = process_DAMOED(obs_mask, args, pre_entity_dropout, pre_entity_combination, env.get_env_info(args), env.get_state())
                ...
                if args.twoagent:
                    send_dict["obs"] = process_agent_obs(args, args.agentnames[0], env.get_env_info(args), env.get_state()) + process_agent_obs(args, args.agentnames[1], env.get_env_info(args), env.get_state())
                else:
                    send_dict["obs"] = process_agent_obs(args, args.agentnames[0], env.get_env_info(args), env.get_state()) 
            ...
            if entity_scheme: # REFIL / CAMA
                ...
                if args.twoagent:
                    ...
                    "avail_actions": env.get_avail_actions(args.n_agents),
                    "obs": process_agent_obs(args, args.agentnames[0], env.get_env_info(args), env.get_state()) + process_agent_obs(args, args.agentnames[1], env.get_env_info(args), env.get_state())
                    })
                else:
                    remote.send({
                    "state": process_state(args, env.get_env_info(args), env.get_state()),
                    "avail_actions": env.get_avail_actions(args.n_agents),
                    "obs": process_agent_obs(args, args.agentnames[0], env.get_env_info(args), env.get_state())
                    })
