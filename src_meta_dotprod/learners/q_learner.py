...
class QLearner:
    ...
    def train_meta_dotprod(self, buffer_list: List[ReplayBuffer], t_env: int, episode_num: int):
        ...
        for i in range(len(buffer_list) // 2):
            ...
            G1 = [g / g.norm() if g.norm() > 1 else g for g in G1]
            ...
            G2 = [g / g.norm() if g.norm() > 1 else g for g in G2]
            ...
            G2_1 = [g / g.norm() if g.norm() > 1 else g for g in G2_1]

            G1G2 = [(g2_1 - g2) / alpha for g2_1, g2 in zip(G2_1, G2)]
            G1G2 = [g / g.norm() if g.norm() > 1 else g for g in G1G2]
            
            G = [g1 + g2 + LR_G1G2 * g1g2 for g1, g2, g1g2 in zip(G1, G2, G1G2)]
            G = [th.clamp(g, -10, 10) for g in G]
            
            for param, g in zip(self.params, G):
                param.grad = LR * g

            grad_norm = th.nn.utils.clip_grad_norm_(self.params, self.args.grad_norm_clip)  # Clip the gradients
            self.optimiser.step()
            self.optimiser.zero_grad()
