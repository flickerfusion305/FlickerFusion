...
class EpisodeRunner:
    ...
    def __init__(self, args, logger):
        ...
        if self.args.render_atten: 
            if self.args.render_atten_ts == 0: 
                self.args.render_atten_ts = int(self.args.env_args["scenario_config"]["episode_limit"]) /2
            self.args.render_now = False
        if self.args.entity_dropout_train or self.args.entity_dropout_inference:
            ...
            if self.args.test_dropout_domain_aware:
                self.args.env_args["scenario_config"]["domain_aware"] = True
            else:
                self.args.env_args["scenario_config"]["domain_aware"] = False
        else:
    ...
    def _get_pre_transition_data(self, test=False):
        if self.args.entity_scheme: # REFIL / CAMA
            ...
            if self.args.entity_dropout:
                dropout_obs_mask, dropout_timestep, entity_order, entity_dropout, entity_combination, temporalcnt = process_EntityDropoutAtten(
                    ...
                    self.env.get_env_info(self.args), self.env.get_state(), self.temporalcnt, test)
    ...
    def run(self, test_mode=False, test_scen=None, index=None, vid_writer=None, domain_n=0, **kwargs):
        ...
        if self.args.test_unseen:
            ...
            constrain_num=None
        if self.args.render and self.args.evaluate: 
            print(f"episode: {len(self.test_returns)}")
        ...
        while not terminated:
            pre_transition_data = self._get_pre_transition_data(test=test_mode)
            ...
            reward, terminated, additional, env_info = self.env.step(actions[0].cpu(), pre_transition_data)
            if vid_writer is not None:
                vid_writer.append_data(self.env.render())
            elif self.args.render:
                if self.args.save_gif: 
                    self.args.render_args['mode'] = 'rgb_array'
                if self.args.entity_dropout: 
                    self.env.render(**self.args.render_args, render_flicker = self.args.render_flicker, entity_dropout=self.pre_entity_dropout)
                else: 
                    self.env.render(**self.args.render_args)

            if self.args.render_atten:
                if self.t == self.args.render_atten_ts-1:
                    self.args.render_now = True 
                if self.t == self.args.render_atten_ts: 
                    a = input("press s for attention:")
                    if  a == "s":
                        labels = ["OOD1", "OOD2"]
                        ...
                        if self.args.dropout_rainbow: 
                            ...
                            model_name = "qmix"
                        plotname = f"{env_name}-{label}_attention_matrix-{model_name}.png"
                        plt.savefig(plotname, dpi=600, bbox_inches='tight')
                        # plt.show()
                        self.args.render_now = False 
                        self.args.render_atten = False 
                        return
                    else:
                        plt.close("all")
                        self.args.render_now = False 
        ...
        if test_mode and (len(self.test_returns) == self.args.test_nepisode):
            ...
            self.env.save_render(self.args.save_gif_name+".gif")
