...
class MultiAgentFluidEnv(gym.Env, MultiAgentEnv):
    ...
    def enhance_color(self, rgb_array, saturation_factor=1.5, brightness_factor=1.0):
        # Normalize the RGB values to the [0, 1] range
        hue, sat, val = colorsys.rgb_to_hsv(*rgb_array)

        # Increase saturation and brightness if the color is not pure black
        if val > 0:
            sat = np.clip(sat * saturation_factor, 0, 1)
            val = np.clip(val * brightness_factor, 0, 1)

        # Convert back to RGB
        r, g, b = colorsys.hsv_to_rgb(hue, sat, val)

        # Return the enhanced color as a numpy array
        return np.array([r, g, b])

    # render environment
    def render(self, mode='human', srk=[1,2,4], sr_color=["#2CAFAC","#FB5607", "#1982C4"], render_flicker=False, entity_dropout=None):
        entity_list = []
        ...
        obs_dropout_list = []
        if entity_dropout is not None and 'obs' in self.pre_transition_data:
            entity_dropout_0 = entity_dropout[0]
            for entity_type in entity_dropout_0: 
                for index in entity_dropout_0[entity_type]:
                    obs_dropout_list.append(f"{entity_type}_{index}")
        # print(obs_dropout_list)
        if mode == 'human':
        ...
        vis_entity_names = [entity.name for entity in self.world.entities]
        if self.pre_transition_data and render_flicker:
            if 'obs_mask' in self.pre_transition_data:
                vis_entity_names = [entity_name for idx, entity_name in enumerate(entity_list) if not self.pre_transition_data['obs_mask'][0][0][idx]]
            elif 'obs' in self.pre_transition_data: 
                vis_entity_names = [entity_name for idx, entity_name in enumerate(entity_list) if entity_name not in obs_dropout_list]
        ...
        for entity in self.world.entities:
            ...
            if entity.name not in vis_entity_names:
                geom.set_color(*entity.color, alpha=0.2)
            else:
                if entity.name == vis_entity_names[0] and render_flicker: 
                    color = self.enhance_color(entity.color, 2.0, 0.7)
