...
class EntityAttentionLayer(nn.Module):
    ...
    def forward(self, entities, pre_mask=None, post_mask=None, ret_attn_logits=None, ret_attn_weights=False, rank_percent=None, entity_mask=None):
        ...
        n_queries = post_mask.shape[1] #na: number of agents
        
        ne, bs, ed = entities_t.shape
        query, key, value = self.in_trans(entities_t).chunk(3, dim=2) #ne*bs*ed  * 3

        # selects only the first n_queries (i.e., na) elements of the query, resulting in shape (na, bs, embed_dim)
        # AGENT TOKENS MUST BE GIVEN FIRST
        ...
        if pre_mask is not None:
            ...
            if rank_percent is not None: # where CAMA attention masking happen
        ...
        if self.args.render_atten and self.args.render_now:
            # fig, ax = plt.subplots()
            # attn_weights_np = np.mean(attn_weights.detach().numpy(), axis=0)
            # cax = ax.imshow(attn_weights_np, cmap='viridis', interpolation='nearest')
            # for i in range(attn_weights_np.shape[0]):
            #     for j in range(attn_weights_np.shape[1]):
            #         ax.text(j, i, f'{attn_weights_np[i, j]:.2f}', ha='center', va='center', color='white')
            fig, ax = plt.subplots(figsize=(10, 4), constrained_layout=True)  # Increased figure size
            attn_weights_np = (np.mean(attn_weights.detach().numpy(), axis=0)*100)
            attn_weights_np = attn_weights_np.astype(int)
            cax = ax.imshow(attn_weights_np, cmap='viridis', interpolation='nearest', aspect='equal')
            ...
            # add lines
            ax.set_xticks(np.arange(attn_weights_np.shape[1]) + 0.5, minor=True)
            ax.set_yticks(np.arange(attn_weights_np.shape[0]) + 0.5, minor=True)
            ax.grid(which="minor", color="white", linestyle='-', linewidth=1)
            ax.tick_params(which="minor", size=0)
            ax.xaxis.set_major_locator(MaxNLocator(integer=True))
            labels = ["OOD1", "OOD2"]
            label_idx = self.args.env_args["scenario_config"]["empirical_study"]-1
            if label_idx >= 2: 
                label_idx -= 2
            label = labels[label_idx]
            for i in range(attn_weights_np.shape[0]):
                for j in range(attn_weights_np.shape[1]):
                    text_value = int(attn_weights_np[i, j])
                    background_color = cax.cmap(cax.norm(attn_weights_np[i, j]))
                    ...
                    ax.text(j, i, f'{text_value}', ha='center', va='center', color=font_color, fontsize=font_size[label][0])
            env_name = self.args.env_args['scenario_id'][7:-3]
            if self.args.dropout_rainbow: 
                model_name = "FlickerFusion"
            else: 
                model_name = "QMIX"

            ax.set_title(f"{env_name.title()}: {label} {model_name} Attention Matrix (t := {int(self.args.render_atten_ts)})", fontsize=font_size[label][1], fontweight="bold")
            ax.set_xlabel('Entities (incl. Agents)', fontsize=font_size[label][2], fontweight='bold')
            ax.set_ylabel('Agents', fontsize=font_size[label][2], fontweight='bold')
            cbar = fig.colorbar(cax, ax=ax, shrink=1.0, aspect=18, pad=0.02) 
