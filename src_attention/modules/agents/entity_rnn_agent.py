...
class EntityAttentionRNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        ...
        #                                    args.pooling_type,
        #                                    args)
        self.fc2 = nn.Linear(args.attn_embed_dim*(1+args.self_loc+args.double_attn), args.rnn_input_dim)
    ...
    def forward(self, inputs, hidden_state, ret_attn_logits=None, msg=None, ret_attn_weights=False):
        ...
        hs = []
        for t in range(ts):
            curr_x3 = x3[:, t].reshape(-1, self.args.rnn_input_dim)
        ...
        if ret_attn_weights is True:
            return q, hs, self.attn_weights
        return q, hs
