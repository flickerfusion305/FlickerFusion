...
def evaluate_sequential(args, runner, logger, load_time_step=0):
    ...
    for i in range(n_scen):
        run_args = {'test_mode': True, 'vid_writer': vw,
                    'test_scen': True, 'domain_n': args.env_args['scenario_config']['empirical_study']}
        if args.eval_all_scen:
            run_args['index'] = i
        for j in range(n_test_batches):
            batch = runner.run(**run_args)
            if args.save_entities_and_msg:
                can_close_env = True
                pad_msg = np.pad(runner.mac.agent.msg, ((0,0),(0,args.env_args['episode_limit']+1-runner.mac.agent.msg.shape[1]),(0,0),(0,0)))
                if j == 0:
                    dic = {'entities':batch['entities'].detach().cpu().numpy(),
                            'obs_mask':batch['obs_mask'].detach().cpu().numpy(),
                            'msg': pad_msg}
                else:
                    dic['entities'] = np.concatenate([dic['entities'], batch['entities'].detach().cpu().numpy()], axis=0)
                    dic['obs_mask'] = np.concatenate([dic['obs_mask'], batch['obs_mask'].detach().cpu().numpy()], axis=0)
                    dic['msg'] = np.concatenate([dic['msg'], pad_msg], axis=0)
        if args.save_entities_and_msg:
            token = osp.split(args.checkpoint_path)[-1]
            save_path = os.path.join(args.local_results_path, "msg", token)
            os.makedirs(save_path, exist_ok=True)
            file_name = osp.join(save_path, str(load_time_step)+".pkl")
            with open(file_name, "wb") as f:
                pickle.dump(dic, f)
            logger.console_logger.info("Saving message and observations to {}".format(file_name))
        rm = runner.rm
...
def run_sequential(args, logger):
    ...
    if not args.entity_scheme:
        ...
        args.private_entity_shape = env_info.get("private_entity_shape", 0)
        # Entity scheme
        scheme = {
