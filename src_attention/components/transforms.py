...
def process_EntityDropoutObs(args, agentname, dropout_timestep, pre_order, pre_dropout, pre_combination, env_info, state_global, temporalcnt):
    dropout_timestep += 1
    onehot_dict = env_info["onehot_dict"]
    entity_maxnum = env_info["entity_maxnum"]
    entitynames = list(onehot_dict.keys())
...
def getEntityDropoutAtten(args, comb_idx, entitynums, entitysize, state_global, entitynames, env_info, cur_comb, cur_order, temporalcnt, attention_values=None):
    entity_maxnum = env_info["entity_maxnum"]
    ...
    elif args.dropout_type == "attention" and attention_values is not None:
        for aidx in range(entitynums['agent']):
            agent_do = np.zeros((entitysize), dtype=np.int8)
            if f"agent-pos_{aidx}" in state_global.keys():
                for enname in entitynames:
                    if enname in env_info['dropout_num'].keys():
                        if cur_comb[enname] in env_info["dropout_num"][enname].keys():
                            donum = env_info["dropout_num"][enname][cur_comb[enname]]
                            samples = [(enidx, attention_values[aidx][add_entity_index[enname] + enidx]) 
                                       for enidx in comb_idx[enname] if not (enname == "agent" and enidx == aidx)]
                            samples = sorted(samples, key=lambda x: x[1])

                            for maskidx, _ in samples[:donum]:
                                agent_do[add_entity_index[enname] + maskidx] = 1
            cur_dropout.append(agent_do)
    return cur_dropout


def process_EntityDropoutAtten(obs_mask, args, dropout_timestep, pre_order, pre_dropout, pre_combination, env_info, state_global, temporalcnt, attention_values=None):
    dropout_timestep += 1
    onehot_dict = env_info["onehot_dict"]
    entity_maxnum = env_info["entity_maxnum"]
    entitynames = list(onehot_dict.keys())
    ...
    if pre_combination != cur_comb:
        ...
        cur_dropout = getEntityDropoutAtten(args, comb_idx, entitynums, entitysize, state_global, entitynames, env_info, cur_comb, cur_order, temporalcnt, attention_values=attention_values)
        obs_mask = obs_mask_dropout(obs_mask, cur_dropout)
        dropout_timestep = 0
    
    elif args.dropout_flicker == True and dropout_timestep == args.flicker_interval: # No order change, but Dropout change
        cur_order = pre_order
        cur_dropout = getEntityDropoutAtten(args, comb_idx, entitynums, entitysize, state_global, entitynames, env_info, cur_comb, cur_order, temporalcnt, attention_values=attention_values)
        obs_mask = obs_mask_dropout(obs_mask, cur_dropout)
        dropout_timestep = 0
